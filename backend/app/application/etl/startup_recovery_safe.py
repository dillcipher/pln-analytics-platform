...
RECOVERABLE_STATUSES = {"UPLOADED", "DETECTING", "VALIDATING", "MERGING", "TRANSFORMING", "EXPORTING", "ASSEMBLY_QUEUED", "ASSEMBLY_COMPLETED", "FAILED"}
MAX_FAILED_RECOVERY_ATTEMPTS = max(1, int(os.getenv("MAX_FAILED_RECOVERY_ATTEMPTS", "3")))
RECOVERY_POLICY_VERSION = "2026-08-23-v4"
_LOCK = asyncio.Lock()
...
def _client():
    if not S3_ENDPOINT or not S3_ACCESS_KEY_ID or not S3_SECRET_ACCESS_KEY:
        return None
    return boto3.client("s3", endpoint_url=S3_ENDPOINT, region_name=S3_REGION, aws_access_key_id=S3_ACCESS_KEY_ID, aws_secret_access_key=S3_SECRET_ACCESS_KEY, config=Config(signature_version="s3v4", retries={"max_attempts": 5, "mode": "adaptive"}, connect_timeout=30, read_timeout=600, s3={"addressing_style": "path"}))
...
async def _reject(metadata: dict[str, Any], job_id: str, reason: str) -> None:
    metadata.update({"status": "REJECTED", "progress": 0, "current_step": "RECOVERY_REJECTED", "last_error": reason, "rejected_at": datetime.now().isoformat(), "recovery_policy_version": RECOVERY_POLICY_VERSION})
    try:
        await _persist(metadata)
    except Exception:
        logger.exception("STARTUP RECOVERY: failed persisting rejected state | job=%s", job_id)


def _manifest_dataset(job_id: str) -> str | None:
    try:
        manifest = json.loads((RAW_UPLOAD / job_id / "manifest.json").read_text(encoding="utf-8"))
...
async def _recover_one(metadata: dict[str, Any]) -> str:
    ...
    if status == "FAILED" and last_error.startswith("Unable to detect dataset for uploaded file(s):"):
        await _reject(metadata, job_id, last_error)
        logger.warning("STARTUP RECOVERY: terminal undetectable job rejected | job=%s | file=%s", job_id, filename)
        return "rejected"
    ...
    if attempts >= MAX_FAILED_RECOVERY_ATTEMPTS:
        if previous_policy != RECOVERY_POLICY_VERSION:
            ...
            logger.warning("STARTUP RECOVERY: retry limit reached | job=%s | attempts=%s", job_id, attempts)
            return "failed"
    ...
    try:
        ...
        upload_id = file_meta.get("upload_id") or metadata.get("upload_id")
        total_chunks = file_meta.get("total_chunks") or metadata.get("total_chunks")
        try:
            result = await UploadService.recover_assembled_job(job_id=job_id, filename=filename, content_type=content_type)
        except FileNotFoundError:
            if not upload_id or total_chunks is None:
                raise
            result = await UploadService.assemble_chunk_upload(upload_id=str(upload_id), job_id=job_id, filename=filename, total_chunks=int(total_chunks), content_type=content_type)
        ...
        if not dataset or dataset == "UNKNOWN":
            reason = f"Unable to detect dataset for uploaded file(s): {filename}. Use a supported PLN dataset or upload a valid Excel workbook."
            await _reject(metadata, job_id, reason)
        ...
        metadata.update({"status": "FINISHED", "progress": 100, "current_step": "FINISHED", "finished_at": datetime.now().isoformat(), "recovery_completed_at": datetime.now().isoformat()})
        metadata.pop("last_error", None)
        await _persist(metadata)
        logger.info("STARTUP RECOVERY: job=%s finished successfully", job_id)
        return "recovered"
    except Exception as exc:
        metadata.update({"status": "FAILED", "current_step": "RECOVERY_FAILED", "last_error": str(exc), "last_failed_at": datetime.now().isoformat()})
        try:
            await _persist(metadata)
        except Exception:
...
async def recover_pending_jobs() -> dict[str, int]:
    async with _LOCK:
        ...
        if not jobs:
            ...
            return {"found": 0, "recovered": 0, "failed": 0, "rejected": 0}
        ...
        for metadata in jobs:
            ...
            if result == "recovered":
                ...
                failed += 1
        logger.info("STARTUP RECOVERY COMPLETED | found=%s recovered=%s failed=%s rejected=%s", len(jobs), recovered, failed, rejected)
        return {"found": len(jobs), "recovered": recovered, "failed": failed, "rejected": rejected}
