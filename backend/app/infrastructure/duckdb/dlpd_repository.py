...
class DuckDbDlpdRepository(DlpdRepository):
    ...
    def _inspection_cte() -> str:
        ...
        warehouse. That is unnecessarily expensive for "Semua Bulan".
        DuckDB's ARG_MAX keeps the same logical result while avoiding the
        large window sort.
            ...
            pengecekan_valid AS (
                ...
                WHERE IDPEL IS NOT NULL
                  AND ((LATITUDE BETWEEN {lat_min} AND {lat_max} AND LONGITUDE BETWEEN {lon_min} AND {lon_max})
                       OR (LATITUDE BETWEEN {lon_min} AND {lon_max} AND LONGITUDE BETWEEN {lat_min} AND {lat_max}))
            )
